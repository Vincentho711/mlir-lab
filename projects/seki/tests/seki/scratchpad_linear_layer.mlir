...
func.func @scratchpad_linear_layer_tensor(
    ...
    %spad = memref.alloc() {alignment = 64 : i64} : memref<4x3xf32, #seki.memory_space<scratchpad>>

    // CHECK: linalg.fill
    linalg.fill ins(%zero : f32) outs(%spad : memref<4x3xf32, #seki.memory_space<scratchpad>>)
    // CHECK: linalg.matmul
    linalg.matmul ins(%X, %W : memref<4x8xf32>, memref<8x3xf32>) 
                  outs(%spad : memref<4x3xf32, #seki.memory_space<scratchpad>>)

    // CHECK: linalg.generic
    linalg.generic {
        indexing_maps = [#map, #map1],
        iterator_types = ["parallel", "parallel"]
    } ins(%b : memref<3xf32>) outs(%spad : memref<4x3xf32, #seki.memory_space<scratchpad>>) {
    ...
    // CHECK: memref.copy
    memref.copy %spad, %result : memref<4x3xf32, #seki.memory_space<scratchpad>> to memref<4x3xf32>
